...
extension SwiftyTeeth {
    public func retrievePeripherals(withIdentifiers uuids: [UUID]) -> [Device] {
...
extension SwiftyTeeth {
    ...
    public func scan(changes: ((Device) -> Void)?) {
        scanChangesHandler = changes
        scan()
    }

    public func scan(
        for timeout: TimeInterval = 10, changes: ((Device) -> Void)? = nil,
        complete: @escaping ([Device]) -> Void
    ) {
        scanChangesHandler = changes
...
extension SwiftyTeeth: CBCentralManagerDelegate {
    ...
    public func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        Log(
            v:
                "centralManager: didConnect to \(peripheral.identifier) with maximum write value of \(peripheral.maximumWriteValueLength(for: .withoutResponse)) and \(peripheral.maximumWriteValueLength(for: .withResponse))"
        )
...
extension SwiftyTeeth: CBPeripheralDelegate {
    ...
    public func peripheral(
        ...
        connectedDevices[peripheral.identifier.uuidString]?.didModifyServices(
    ...
    public func peripheral(
        ...
        connectedDevices[peripheral.identifier.uuidString]?.didDiscoverIncludedServicesFor(
    ...
    public func peripheral(
        ...
        connectedDevices[peripheral.identifier.uuidString]?.didDiscoverCharacteristicsFor(
            service: service, error: error)
    }
    ...
    public func peripheral(
        ...
        connectedDevices[peripheral.identifier.uuidString]?.didUpdateValueFor(
    ...
    public func peripheral(
        ...
        connectedDevices[peripheral.identifier.uuidString]?.didWriteValueFor(
            characteristic: characteristic, error: error)
    }
    ...
    public func peripheral(
        ...
        connectedDevices[peripheral.identifier.uuidString]?.didUpdateNotificationStateFor(
            characteristic: characteristic, error: error)
    }
    ...
    public func peripheral(
        ...
        connectedDevices[peripheral.identifier.uuidString]?.didDiscoverDescriptorsFor(
            characteristic: characteristic, error: error)
    }
